    ...
    try:
        ...
        for epoch in itertools.count(init_epoch+1):
            if epoch % hp.log.validation_interval == 0:
                with torch.no_grad():
                    validate(hp, args, model_g, model_d, valloader, stft_loss,
                             sub_stft_loss, criterion, pqmf, writer, step)
            ...
            for (melG, audioG), (melD, audioD) in loader:
                ...
                if hp.model.use_subband_stft_loss:
                    ...
                    # (B, C, T) -> (B x C, T)
                    y_mb_ = y_mb_.view(-1, y_mb_.size(2))
                    sub_sc_loss, sub_mag_loss = sub_stft_loss(
                        y_mb_[:, :y_mb.size(-1)], y_mb)  # y_mb --> [B*C, T]
                ...
                if step > hp.train.discriminator_train_start_steps:
                    ...
                    for _ in range(hp.train.rep_discriminator):
                        ...
                        for (_, score_fake), (_, score_real) in zip(disc_fake, disc_real):
                            loss_d_real += criterion(score_real,
                                                     torch.ones_like(score_real))
                            loss_d_fake += criterion(score_fake,
                                                     torch.zeros_like(score_fake))
                        loss_d_real = loss_d_real / \
                            len(disc_real)  # len(disc_real) = 3
                        loss_d_fake = loss_d_fake / \
                            len(disc_fake)  # len(disc_fake) = 3
